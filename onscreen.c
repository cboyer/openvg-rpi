/* OPENVG ONSCREEN RENDERING EXAMPLE FOR RASPBERRY PI
 * C.Boyer, 2020
 * 
 * Needed libraries: OpenVG, EGL, OpenGL ES, Freetype2, Glib2, stb
 * Compile with:
 * gcc onscreen.c -O2 -Wall -Werror -L/opt/vc/lib -lbrcmEGL -lbrcmGLESv2 -lbcm_host -I/opt/vc/include -I/opt/vc/include/interface/vmcs_host/linux -I/opt/vc/include/interface/vcos/pthreads  -I/usr/include/stb -lstb -I/usr/include/freetype2 -lfreetype -I/usr/include/glib-2.0 -I/usr/lib/arm-linux-gnueabihf/glib-2.0/include -lglib-2.0
 */
...
void text_onscreen() {
    ...
    VGPath path = vgCreatePath(VG_PATH_FORMAT_STANDARD, VG_PATH_DATATYPE_F, 1.0f, 0.0f, 0.0f, 0.0f, VG_PATH_CAPABILITY_ALL);
    void *ScreenBuffer = malloc(screen_width * screen_height * 4); // 4 = channel num
    char filename[100] =  "";

    for(int i = 0; i < 3; i++) {
        //eglSwapBuffers(p_state->display, p_state->surface); //On screen printing and stbi_write_png
        ...
        //eglSwapBuffers(p_state->display, p_state->surface); //On screen printing only
